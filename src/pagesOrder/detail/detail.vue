...
import { deleteMemberOrderAPI, getMemberOrderByIdAPI, getMemberOrderConsignmentByIdAPI, getMemberOrderLogisticsByIdAPI, putMemberOrderReceiptByIdAPI, getMemberOrderCancelByIdAPI } from '@/services/order'
...
onReady(() => {
  ...
  //动画效果,导航栏标题
  pageInstance.animate('.navbar .title', [{ color: 'transparent' }, { color: '#000' },
    1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
...
const onOrderSend = async () => {
  console.log('on order send:' + query.id)
...
<template>
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower" enable-back-to-top>
    <template v-if="true">
      <!-- 订单状态 -->
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button-group">
            ...
            <view v-if="isDev && order?.orderState == OrderState.DaiFaHuo" class="button" @tap="onOrderSend"> 模拟发货
            </view>
          </view>
        </template>
      </view>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button delete" v-if="order.orderState >= OrderState.DaiPingJia" @tap="onOrderDelete"> 删除订单
          </view>
        </template>
      </view>
    </template>
